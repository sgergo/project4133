...
static void uartstdio_primetransmit(void) {
    ...
    if(!TX_BUFFER_EMPTY) {
        ...
        while(!TX_BUFFER_EMPTY) {
            ...
            board_consoleuart_write_blocking((uint8_t)uarttxbuffer[uarttxreadindex]);
...
void uartstdio_config(defuint_t portnum, defuint_t baud, defuint_t clockfreq) {
    ...
    //board specific initialisation is in 'board_consoleuart.c'
}
...
void uartstdio_rx_isr(void) {
    unsigned char character;
    defuint_t store;
    static bool lastwas_carriage_return = false;
    
    // Read a character
    store = board_consoleuart_getcharfromrxbuf();
    ...
    if(!g_disable_echo) {
        ...
        if(character == '\b') {
            ...
            // Return to wait for the next character.
            return;
        }
